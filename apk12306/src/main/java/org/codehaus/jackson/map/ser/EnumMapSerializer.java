  ...
  implements ResolvableSerializer
{
  ...
  {
    ObjectNode localObjectNode = createSchemaNode("object", true);
    if ((paramType instanceof ParameterizedType))
    {
      ...
      if (localObject.length == 2)
      {
        ...
        JavaType localJavaType = paramSerializerProvider.constructType(localObject[1]);
        localObject = JsonNodeFactory.instance.objectNode();
        ...
        if (i < j)
        {
          ...
          paramType = paramSerializerProvider.findValueSerializer(localJavaType.getRawClass(), this._property);
          ...
          {
            ((ObjectNode)localObject).put(paramSerializerProvider.getConfig().getAnnotationIntrospector().findEnumValue(localEnum), paramType);
            i++;
            break;
          }
        }
        localObjectNode.put("properties", (JsonNode)localObject);
      }
    }
...
/* Location:              E:\soft\ApkIDE\Worksrc\com.MobileTicket\!\org\codehaus\jackson\map\ser\EnumMapSerializer.class
