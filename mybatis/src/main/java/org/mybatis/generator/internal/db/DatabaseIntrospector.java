...
public class DatabaseIntrospector {
    ...
    private void calculatePrimaryKey(FullyQualifiedTable table,
        ...
        try {
            ...
            while (rs.next()) {
                String columnName = convertColumnName(rs.getString("COLUMN_NAME")); //$NON-NLS-1$
...
//            columnName = columnName.replace("fk","").replace("_","").toLowerCase() + "Id";
//        }
        return columnName;
    }
    /**
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        while (rs.next()) {
            ...
            introspectedColumn.setActualColumnName(convertColumnName(rs.getString("COLUMN_NAME"))); //$NON-NLS-1$
            introspectedColumn
        ...
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
            ...
            FullyQualifiedTable table = new FullyQualifiedTable(
                    ...
                    stringHasValue(tc.getSchema()) ? atn
                            .getSchema() : null,
                    atn.getTableName(),
//                    convertTableName(atn.getTableName()),
    ...
    private String convertTableName(String t1){
        String [] t2 = t1.split("_");
        StringBuffer sb=  new StringBuffer();
        for(String s : t2){
            String ss = String.valueOf(s.charAt(0));
            s = s.replace(ss,ss.toUpperCase());
            sb.append(s);
        }
        return sb.toString();
    }
