...
public class HeapSort {
     ...
     *
     */
    public static void maxHeap(int[] array, int s, int length) {
        int tmp = array[s];
     ...
     *
     */
    public static void minHeap(int[] array, int s, int length) {
        int tmp = array[s];
        int child = 2 * s + 1;// 左孩子结点的位置
        System.out.println("待调整结点为：array[" + s + "] = " + tmp);
        while (child < length) {
...
//        }
//        System.out.println("build heap sort:"+ ArrayUtils.toString(array));
...
//        }
    ...
    public static void buildMaxHeap(int [] arr, int index, int length){
        System.out.println("sort index = " + index);
        int left = index << 1 + 1;
        int right = index << 1 + 2;
        int maxm = index;
        ...
        if(right <  length && arr[right] > arr[maxm]){
            maxm = right;
        }
        if(maxm == index){
            return;
        ...
        arr[index] = arr[maxm];
        arr[maxm] = t;
        buildMaxHeap(arr,maxm,length);
    }

    public static void adjustMaxHeap(){
